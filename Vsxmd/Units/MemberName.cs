...
namespace Vsxmd.Units
{
    ...
    internal class MemberName : IComparable<MemberName>
    {
        ...
        internal string Caption =>
            this.Kind == MemberKind.Type
            ? $"{this.Href.ToAnchor()}## " + "`" + this.Kind.ToLowerString() + "`" + $" {this.FriendlyName.Escape()} {this.Href.ToHereLink()} {TableOfContents.Link}"
            : this.Kind == MemberKind.Constants ||
              this.Kind == MemberKind.Property
            ? $"{this.Href.ToAnchor()}### `{this.Kind.ToLowerString()}` {this.FriendlyName.Escape()} {this.Href.ToHereLink()} {TableOfContents.Link}"
            : this.Kind == MemberKind.Constructor ||
              this.Kind == MemberKind.Method
            ? $"{this.Href.ToAnchor()}### `{this.Kind.ToLowerString()}` {this.FriendlyName.Escape()}({this.paramNames.Join(",")}) {this.Href.ToHereLink()} {TableOfContents.Link}"
            : string.Empty;
